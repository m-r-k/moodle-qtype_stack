 ...
 * This class adds in the "adapt button" blocks to castext.
 */
class stack_cas_castext2_adaptreplacebutton extends stack_cas_castext2_block {
    ...
    public function compile($format, $options): ?MP_Node {
        ...
        $input_value = isset($this->params['input_value']) ? $this->params['input_value'] : 'true';
        ...
        $code .= "if (input.value=='true'){ replace(); }\n";
        ...
        $list[] = new MP_String("function replace(){");
    ...
    public function validate(&$errors=[], $options=[]): bool {
        ...
        if (!array_key_exists('replace_ids', $this->params)) {
            $errors[] = new $options['errclass']('Adaptbutton block requires a replace_ids parameter.',
                $options['context'] . '/' .
                $this->position['start'] . '-' . $this->position['end']);
            return false;
        }
